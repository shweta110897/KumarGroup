...
public class RC_Financer_Detail_Activity extends AppCompatActivity {

    ProgressDialog pro;
    RecyclerView rclvPlateUpdate;
    RC_Financer_Update_Detail_Adapter numberPlate_update_detail_adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        WebService.getClient().GetRCUpdateFinancerList(emp).enqueue(new Callback<RcUpdate>() {
            @Override
            public void onResponse(Call<RcUpdate> call, Response<RcUpdate> response) {
                ...
                if (response.body().getData().size()==0 || response.body().getMessage().equals("No users found.")){
                    Toast.makeText(RC_Financer_Detail_Activity.this, "No Data", Toast.LENGTH_SHORT).show();
                }else {
                    data = response.body().getData();
                    rclvPlateUpdate.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
                    numberPlate_update_detail_adapter = new RC_Financer_Update_Detail_Adapter(RC_Financer_Detail_Activity.this, RC_Financer_Detail_Activity.this, data);
