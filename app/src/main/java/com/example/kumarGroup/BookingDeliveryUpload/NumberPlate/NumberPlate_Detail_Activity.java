...
public class NumberPlate_Detail_Activity extends AppCompatActivity {

    ProgressDialog pro;
    RecyclerView rclvPlateUpdate;
    Number_Plate_Update_Detail_Adapter numberPlate_update_detail_adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        WebService.getClient().GetNumberPlateFitmentList(emp).enqueue(new Callback<RcUpdate>() {
            @Override
            public void onResponse(Call<RcUpdate> call, Response<RcUpdate> response) {
                ...
                if (response.body().getData().size()==0 || response.body().getMessage().equals("No users found.")){
                    Toast.makeText(NumberPlate_Detail_Activity.this, "No Data", Toast.LENGTH_SHORT).show();
                }else {
                    data = response.body().getData();
                    rclvPlateUpdate.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
                    numberPlate_update_detail_adapter = new Number_Plate_Update_Detail_Adapter(NumberPlate_Detail_Activity.this, NumberPlate_Detail_Activity.this,data);
                    rclvPlateUpdate.setAdapter(numberPlate_update_detail_adapter);
                }
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String query) {
                ...
                if (data == null) {
                    ...
                    numberPlate_update_detail_adapter.getFilter().filter(query);
                    // showDetailGVAdapter.getFilter().filter(query);
                    Log.d("ssss", "onQueryTextChange: "+ numberPlate_update_detail_adapter);
